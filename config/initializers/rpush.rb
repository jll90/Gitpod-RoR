IOS_APP = 'truekaso-ios'
ANDROID_APP = 'truekaso-android'

# rpush tiene apps, entonces cada app contiene devices
# hay que crear 2 apps, una para ios y otra para android
# y esas apps se utilizan en el modelo notification según el tipo de device
...
Rpush.configure do |config|
  ...
  config.push_poll = Rails.env.development? ? 10 : 2
...
## This thing fires by default all the time and it's very cumbersome
## When firing rails make sure to pass in RPUSH_ENABLED=true
if ENV['RPUSH_ENABLED'] =='true'
  if defined?(Rails)
...
begin
  android_app = Rpush::Gcm::App.find_by(name: ANDROID_APP)
  if android_app.nil?
    android_app = Rpush::Gcm::App.new
    android_app.name = ANDROID_APP
    android_app.auth_key = ENV['TRUEKASO_FCM_TOKEN']
    android_app.connections = 1
    android_app.save!
  end
rescue => e
  puts "Handled error while creating Rpush Android App #{e}"
end
